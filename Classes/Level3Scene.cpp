...
void Level3::update(float t)  {
    CCLOG("%d", count);
    auto _tileMap = TMXTiledMap::create("TileGameResources/MAPLEVEL3.tmx");
...
bool Level3::init()
{
    ...
    auto Level = Label::createWithTTF("LEV 3", "fonts/BebasNeue-Regular.ttf", 65);
    Level->enableShadow();
    ...
    for (int y = 0; y < _tileMap->getMapSize().height; y++)
    {
        for (int x = 0; x < _tileMap->getMapSize().width; x++)
        {
            ...
            auto id2 = traj->getTileGIDAt(Vec2(2, 10));
            if (id1 == id2) {
...
void Level3::change_color(Sprite* s) {
    if (s->getColor() == Color3B(137, 207, 239)) return;
    s->setColor(Color3B(137, 207, 239));
    Level3::count++;
}
Vec2 Level3::tileCoordForPosition(Point& position, TMXTiledMap* map)
{
...
void Level3::setPlayerPosition(Point position, TMXTiledMap* map, int directionx, int directiony)
{
