...
void Level5::update(float t)  {
    CCLOG("%d", count);
    auto _tileMap = TMXTiledMap::create("TileGameResources/MAPLEVEL5.tmx");
...
bool Level5::init()
{
    ...
    auto Level = Label::createWithTTF("LEV 5", "fonts/BebasNeue-Regular.ttf", 65);
    Level->enableShadow();
...
void Level5::change_color(Sprite* s) {
    if (s->getColor() == Color3B(137, 207, 239)) return;
    s->setColor(Color3B(137, 207, 239));
    Level5::count++;
}
Vec2 Level5::tileCoordForPosition(Point& position, TMXTiledMap* map)
{
...
void Level5::setPlayerPosition(Point position, TMXTiledMap* map, int directionx, int directiony)
{
